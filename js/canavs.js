...
function init() {
  ...
  $('.spot').draggable({
    ...
    stop: ( event, ui ) => {
      var measureBox = $('#measureWindow').position();      
      ipc.send('savePositon', {points: box, infoBox:{x: measureBox.left, y: measureBox.top}});
...
function drawLine() {
      ...
      box[0].x = $(box1).position().left;
      box[0].y = $(box1).position().top;
      box[0].h = 25;
      box[0].w = 25;    


      box[1].x = $(box2).position().left;
      ...
      clear(ctx);
      ctx.beginPath();
      ctx.moveTo( (box[0].x + (box[0].w / 2)) , (box[0].y + (box[0].h / 2)) );
      ctx.lineTo( (box[1].x + (box[1].w / 2)) , (box[1].y + (box[1].h / 2)) );
      ctx.strokeStyle = "#000000";
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo( (box[0].x + (box[0].w / 2) -1) , (box[0].y + (box[0].h / 2) -1) );
      ctx.lineTo( (box[1].x + (box[1].w / 2) -1) , (box[1].y + (box[1].h / 2) -1) );
...
function updateInputs() {
    input1_x.value = box[0].x +' , '+box[0].y;
    //input1_y.value = boxes[0].y;
    input2_x.value = box[1].x +' , '+box[1].y;
    //input2_y.value = boxes[1].y;
    var point1 = new Object;
    var point2 = new Object;
    point1.x = (box[0].x + (box[0].w / 2));
    point1.y = (box[0].y + (box[0].h / 2));
    point2.x = (box[1].x + (box[1].w / 2));
    point2.y = (box[1].y + (box[1].h / 2));
...
ipc.on('startingPoints-reply', function(event, arg) {
    console.log('startingPoints',event, arg);
    startingPoints = arg;
})
